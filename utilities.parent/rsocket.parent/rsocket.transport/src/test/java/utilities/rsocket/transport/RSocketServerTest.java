	...
	public void setUp() throws Exception {
		for (int i = 0; i < SERVERS; i++) {
			serverRef.set(i, new RSocketServer(7000 + i, 1, request -> {
				byte[] original = request.getData().array();
				byte[] metadata = request.getMetadata().array();
				byte[] data = (new String(original) + " world!").getBytes();
		...
		for (int i = 0; i < SERVERS; i++) {
			...
			th[i] = new Thread(() -> {
				...
				for (Future<byte[]> future : list) {
					try {
						String s = new String(future.get());
						assertTrue("this is not [hello world!]", "hello world!".equals(s));
